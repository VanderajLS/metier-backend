...
# Accept both GET and POST so it’s easy to test from browser or curl
@admin_bp.route("/ai/describe", methods=["GET", "POST"])
def ai_describe():
    """
    GET  /api/admin/ai/describe?name=...&sku=...&category=...&specs=...
    POST /api/admin/ai/describe  (JSON body)
    """
    try:
        if request.method == "POST":
            data = request.get_json(force=True) or {}
        else:
            # Build dict from query string for easy testing in browser
            data = {
